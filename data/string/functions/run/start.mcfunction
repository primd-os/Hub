...
# Input Text
summon minecraft:marker ~ ~ ~ {Tags:["string","string.comp"],CustomName:'""',UUID:[I;0,7575123,0,1]}
data modify block -30000000 2 74063 Text1 set value '[{"text":" "},{"storage":"string:in","nbt":"string"},{"text":"          "}]'
data modify entity @e[tag=string.comp,type=marker,limit=1] CustomName set from block -30000000 2 74063 Text1

## Used to binary search the text
# Dynamic Comparison Text
summon minecraft:marker ~ ~ ~ {Tags:["string","string.comp2"],CustomName:'""',UUID:[I;0,7575123,0,2]}

## Used for sorting
# Tag List
summon minecraft:marker ~ ~ ~ {Tags:["string","a","b","c"],CustomName:'""',UUID:[I;0,7575123,0,0]}

## Used to read the sorted list
# Out Compare 
summon minecraft:marker ~ ~ ~ {Tags:["string","string.out"],CustomName:'""',UUID:[I;0,7575123,0,3]}

### Scores
# Set a score for all for easier resetting
scoreboard players set @e[tag=string,type=marker,tag=!string.start] string 0 
tag @e[tag=string,type=marker] remove string
